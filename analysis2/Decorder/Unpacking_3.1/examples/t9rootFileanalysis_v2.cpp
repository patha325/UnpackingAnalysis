  ...
  FileInput->Close();
   ...
   int temp=firstModule;
    ...
    while(!ScinModPos.eof()){
     ...
     TDirectory *modules = wfile.mkdir("modules");
...
//      modules->cd();
     ...
     for (Int_t ih=1; ih <= NumberOfMod;ih++){
        ...
        BeamProf[ih]=new TH2F(sMod.c_str(),sMod.c_str(),  16, 0, 16, 95, -986.2, 986.2);
     }
     ...
     TH2F *EventsMap = new TH2F("All_events_map","All_events_map",  143, 0, 5720, 95, -986.2, 986.2);
     ...
     for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
        ...
        SingleEvent[ih]=new TH2F(sEvent.c_str(),sEvent.c_str(),  143, 0, 5720, 95, -986.2, 986.2);
     ...
     for (Int_t index=0; index <= GTrigTagCoins.size(); index++) {
         for (Int_t mod = 1; mod<=NumberOfMod;mod++){
                 ...
                 auto boundtrigger=std::equal_range (FEB[8].GTrigTag->begin(), FEB[8].GTrigTag->end(), GTrigTagCoins[index]);
                 triggerindex[0] = boundtrigger.first - FEB[8].GTrigTag->begin();
                 triggerindex[1] = boundtrigger.second - FEB[8].GTrigTag->begin();
                 
                  if (leftindex[0]>0 && rightindex[0]>0 && triggerindex[0]>0){
                      if (triggerindex[0]!=triggerindex[1]-1){
                              ...
                              if (FEB[MapCon[1][mod]].hitsChannel->at(CoinsLeft) == FEB[MapCon[2][mod]].hitsChannel->at(CoinsRight)){
                                  if (FEB[MapCon[1][mod]].hitAmpl->at(CoinsLeft) > 500 && FEB[MapCon[2][mod]].hitAmpl->at(CoinsRight)>500) {
                                      if (eventNumber<NumberofEventsMonitor){
                                        ...
                                        SingleEventTime[eventNumber] -> Fill(FEB[MapCon[1][mod]].hitTimeAvr->at(CoinsLeft));
                                        SingleEventTime[eventNumber] -> Fill(FEB[MapCon[2][mod]].hitTimeAvr->at(CoinsRight));
                                        for (Int_t CoinsTop = topindex[0]; CoinsTop < topindex[1]; CoinsTop++){
                                           if (FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop)>=32*MapCon[4][mod] && FEB[MapCon[3][mod]].hitsChannel->at(CoinsTop)<32*(1+MapCon[4][mod])){
                                               if (FEB[MapCon[3][mod]].hitAmpl->at(CoinsLeft) > 300){
                                                SingleEventTime[eventNumber] -> Fill(FEB[MapCon[3][mod]].hitTimeAvr->at(CoinsTop));
     ...
     EventsMap->GetYaxis()->SetTitle("vertical position [mm]");
     EventsMap->GetXaxis()->SetTitle("horizontal position [mm]");           
     EventsMap->Write();
