...
//      modules->cd();
     ...
     for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
        ...
        SingleEvent3D[ih] = new TH3F((sEvent +"_3D").c_str(),(sEvent+"_3D").c_str(),  143, 0, 5720, 16, -1420.5, 1420.5, 95,  -986.2, 986.2);
     }
     ...
     for (Int_t ih=0; ih < NumberofEventsMonitor;ih++){
         if (SingleEvent[ih]->GetEntries()>1) {
          //  SingleEvent3D[ih]->GetYaxis()->SetTitle("vertical position [mm]");
          //  SingleEvent3D[ih]->GetXaxis()->SetTitle("horizontal position [mm]");
