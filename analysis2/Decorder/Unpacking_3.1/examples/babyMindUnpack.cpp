        ...
        for (int i=0; i<nTr; ++i) {
          ...
          for (int ich=0; ich<BM_FEB_NCHANNELS; ++ich) {
              ...
              for (unsigned int ih=0; ih<nlHits; ++ih) {
                  ...
                  //FEB[spill.GetBoardId()].hitGlobalTime.push_back(spill.GetSpillTime()*pow(10,6) + (event->GetTriggerTime()) * pow(10,3) + (event->GetLeadingTime(ih, ich))*2.5 );
...
//       } while (xEv < 5);
  ...
  for (Int_t ih=0; ih<=64; ih++) {
      if (FEB[ih].FEBSN.size()){
        ...
        //FEBtree[ih]->Branch((sFEB+"_hitGlobalTime").c_str(),"vector<double>",&FEB[ih].hitGlobalTime);
      }
  }
