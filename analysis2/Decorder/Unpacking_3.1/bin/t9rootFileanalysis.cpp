...
//      modules->cd();
     ...
     for (Int_t i =0; i < (FEB[MapCon[3][verticcheck]].GTrigTag->size() - 5); i++){
       /*
       if(i<100){
	  cout << FEB[MapCon[3][verticcheck]].GTrigTag->at(i) << " " << FEB[MapCon[3][verticcheck]].GTrigTag->at(i+4) << '\n';
	  }
       */
     ...
     /*
     for(int m=0; m<100; m++){
       cout << GTrigTagCoins[m] << '\n';
     }
     */
     
     TH2F *EventsMap = new TH2F("All_events_map","All_events_map",  143, 0, 5720, 95, -986.2, 986.2);
     TH3D *EventsMap3D = new TH3D ("All_events_map_3D","All_events_map_3D", 143, 0, 5720, 16, -1420.5, 1420.5, 95,  -986.2, 986.2);
     //Int_t EvPerGT =20;
