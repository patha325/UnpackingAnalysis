...
int main ()
{
  ...
  for (int ih = 0; ih<FEBnumbers.size();ih++) {
        ...
        for (Int_t ich=0; ich < 96;ich++){
            ...
            LG_vs_ToT[FEBnumbers.at(ih)][ich]=new TH2F(sCh.c_str(),sCh.c_str(),  36, 0, 35, 4096, 0, 4095);
        }
        
        for (Int_t i = 0; i<FEB[FEBnumbers.at(ih)].hitTimeDif->size(); i++){
            if (FEB[FEBnumbers.at(ih)].hitLGAmpl->at(i)>0 && FEB[FEBnumbers.at(ih)].hitTimeDif->at(i)>0)
        ...
        for (Int_t ich=0; ich < 96;ich++){
            if (LG_vs_ToT[FEBnumbers.at(ih)][ich]->GetEntries()>1) {
                LG_vs_ToT[FEBnumbers.at(ih)][ich]->GetYaxis()->SetTitle("LG [ADC]");
                LG_vs_ToT[FEBnumbers.at(ih)][ich]->GetXaxis()->SetTitle("Time over Threshold [2.5 ns]");
                ...
                LG_vs_ToT[FEBnumbers.at(ih)][ich]->Write();
            }
